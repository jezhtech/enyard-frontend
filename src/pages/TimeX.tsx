...
import { Clock, Camera, Brain, Users, BarChart3, Shield, Play, Code, Terminal, Zap } from "lucide-react";

const TimeX = () => {
  const [typedText, setTypedText] = useState("");
  const [codeLines, setCodeLines] = useState(0);

  const fullText = "import { TimeX } from '@enyard/timex';\n\nconst attendance = new TimeX({\n  ai: true,\n  camera: 'integrated',\n  analytics: 'realtime'\n});\n\nattendance.track().then(result => {\n  console.log('✓ Attendance logged');\n});";
  ...
  const plans = [
    {
      ...
      price: "$29",
      period: "/month",
      features: ["Up to 50 employees", "Basic AI recognition", "Mobile app", "Email support"],
      popular: false
    },
    {
      name: "Professional",
      price: "$99",
      period: "/month",
      features: ["Up to 500 employees", "Advanced AI analytics", "Camera integration", "24/7 support", "Custom reports"],
      popular: true
    },
    {
      name: "Enterprise",
      price: "Custom",
      period: "",
      features: ["Unlimited employees", "Full AI suite", "Multi-location", "Dedicated support", "Custom integrations"],
      popular: false
    }
  ];
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                ...
                <p className="text-xl text-muted-foreground max-w-lg">
                  Revolutionary AI-powered attendance solution with camera integration, 
                  biometric recognition, and real-time analytics.
                </p>
              </div>
              ...
              <Card className="glass border-border/50 overflow-hidden">
                <CardHeader className="bg-muted/30 border-b border-border/50">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-muted-foreground ml-4">timex-demo.js</span>
                  </div>
                </CardHeader>
                <CardContent className="p-6 bg-slate-900/50 font-mono text-sm">
                  <div className="space-y-1">
                    {Array.from({ length: codeLines + 1 }).map((_, i) => (
                      <div key={i} className="flex">
                        <span className="text-muted-foreground/50 w-8 text-right mr-4">{i + 1}</span>
                        <span className="text-green-400">{'>'}</span>
                      </div>
                    ))}
                  </div>
                  <pre className="text-green-400 whitespace-pre-wrap mt-4">
                    {typedText}
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map((feature, index) => (
              <Card key={feature.title} className="group hover-lift glass border-0 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center">
          <div className="glass rounded-2xl p-12">
            ...
            <p className="text-xl text-muted-foreground mb-8">
              Join thousands of companies already using TimeX for seamless attendance management.
            </p>
